...
def plans_to_html(plans: list, data: Data, datetime: datetime.datetime):
    ...
    for index, plan in enumerate(plans):
        ...
        for travel_option in communication:
            ...
            communication_content += f'''<div style="padding: 5px; display: flex; flex-direction: column; justify-content: center; align-items: center;"><img style="height: 23px; width: 23px; margin-bottom: 5px;" src="{image}">{str(travel_option)}</div>'''
...
def prepare_departure_details(plan, start_location: str, goal_location: str, data: Data):
    ...
    for index, plan_trip in enumerate(plan.plan_trips):
        ...
        if index == 0:
            response[0] = f'''<div style="display: flex; width: 90%; align-items: center; margin: 10px 0;"><img style="width: 25px;" src="{static('base_view/img/WALK.png')}"><span style="margin-left: 10px; text-align: left;">{start_stop.name}</span></div>'''

        if index == len(plan.plan_trips) - 1:
            response[len(plan.plan_trips) + 1] = f'''<div style="display: flex; width: 90%; align-items: center; margin: 10px 0;"><img style="width: 25px;" src="{static('base_view/img/WALK.png')}"><span style="margin-left: 10px; text-align: left;">{goal_location}</span></div>'''
